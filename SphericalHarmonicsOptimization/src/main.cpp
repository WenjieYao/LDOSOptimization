...
int main()
{
  ...
  coeffs[0]=d_min/1e3;                          // initial value
  double delta_c = coeffs[0]/500;               // increasement of coefficients used
  for(int i=1;i<Nc;++i)
    coeffs[i] = 0.00;
  //coeffs[1] = 1;
  WriteCoeff(coeffs,num_coeffs);                // write coefficients to file

  double resolution = 1.5/5;                      // resolution, larger the finer, default 1
  ...
  MatrixXd Coeffs(Nc,MAX_ITER);                 // matrix to store all coefficients
